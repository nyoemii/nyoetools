...
class OsuBeatmapView(nextcord.ui.View):
    ...
    async def open_browser(
        ...
        await interaction.response.send_message(
            f"https://osu.ppy.sh/s/{self.beatmap_id}", ephemeral=True
        )
    ...
    async def download_beatmap(
        ...
        await interaction.response.send_message(
            f"osu://dl/{self.beatmap_id}", ephemeral=True
        )
...
class OsuBeatmapConverter(commands.Cog):
    ...
    async def get_access_token(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(
                "https://osu.ppy.sh/oauth/token", data=data
            ) as resp:
                if resp.status == 200:
    ...
    def create_beatmap_embed(self, beatmap_data, beatmap_id):
        ...
        embed.add_field(
            name="⭐ Status",
            value=beatmap_data["status"].replace("_", " ").title(),
            inline=True,
        )
        embed.add_field(
            name="🎯 BPM", value=str(beatmap_data.get("bpm", "N/A")), inline=True
        )
        ...
        if beatmap_data.get("beatmaps") and len(beatmap_data["beatmaps"]) > 0:
            ...
            length_seconds = (
                first_beatmap.get("total_length", 0)
                or first_beatmap.get("hit_length", 0)
                or first_beatmap.get("length", 0)
            )
        ...
        embed.add_field(
            name="🎮 Difficulties", value=str(difficulty_count), inline=True
        )

        if beatmap_data.get("tags"):
            tags = (
                beatmap_data["tags"][:100] + "..."
                if len(beatmap_data["tags"]) > 100
                else beatmap_data["tags"]
            )
            embed.add_field(name="🏷️ Tags", value=tags, inline=False)

        if beatmap_data.get("ranked_date"):
            ranked_date = datetime.fromisoformat(
                beatmap_data["ranked_date"].replace("Z", "+00:00")
            )
            embed.add_field(
                name="📅 Ranked", value=ranked_date.strftime("%Y-%m-%d"), inline=True
            )
    ...
    def create_fallback_embed(self, beatmap_id):
        ...
        embed.add_field(
            name="🌐 View Online",
            value=f"[Click here](https://osu.ppy.sh/s/{beatmap_id})",
            inline=False,
        )
        embed.set_footer(text=f"Beatmapset ID: {beatmap_id}")
        return embed
