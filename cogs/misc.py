...
class Misc(Cog):
    ...
    async def info(self, interaction: Interaction[Bot], ephemeral: bool = False):
        ...
        embed: Embed = Embed(
            title="System Info", color=Colour.blue(), timestamp=datetime.datetime.now()
        )
        embed.add_field(name="CPU Usage", value=f"{psutil.cpu_percent()}%")
        vmem = psutil.virtual_memory()
        embed.add_field(
            name="Memory Usage",
            value=f"{vmem.percent:.1f}% ({size(vmem.total - vmem.available)}/{size(vmem.total)})",
        )
        dusage = psutil.disk_usage("/")
        embed.add_field(
            name="Disk Usage",
            value=f"{dusage.percent:.1f}% ({size(dusage.used)}/{size(dusage.total)})",
        )
        nctrs = psutil.net_io_counters()
        embed.add_field(
            name="Network Stats",
            value=f"Sent: {size(nctrs.bytes_sent)}\nReceived: {size(nctrs.bytes_recv)}",
        )
        ...
        embed.add_field(
            ...
            value="[3.1.0+g0429ea4f](https://github.com/nextcord/nextcord?rev=0429ea4fc2cc7d372425359347c5f8550c41876c#0429ea4fc2cc7d372425359347c5f8550c41876c)",
        )
    ...
    async def credits(self, interaction: Interaction[Bot]):
        embed = nextcord.Embed(title="Credits", color=nextcord.Color.blue())
